...
function BindPadFrame_OnEvent(self, event, ...)
    ...
    if event == "UPDATE_BINDINGS" then
        ...
        -- If we don't save now, we can lose the new keybind when logout/login as other spec.
...
function BindPadCore.PlayerTalentUpdate()
    ...
    BindPadCore.activeTalentGroup = 1
end
...
function BindPadCore.CreateBindPadSlot(usedNumber)
    ...
    BindPadScrollFrameFooter:SetPoint("TOPLEFT", "BindPadSlot" .. (floor((usedNumber - 1) / NUM_SLOTS_PER_ROW) * NUM_SLOTS_PER_ROW + 1), "BOTTOMLEFT", 0, 0)
